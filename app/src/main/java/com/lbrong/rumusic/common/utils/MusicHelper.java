...
public final class MusicHelper {
            ...
            if (cursor.moveToFirst()) {
                for (int i = 0; i < cursor.getCount(); i++) {
                    ...
                    if (isMusic != 0 && duration / (500 * 60) >= 1) {
                        ...
                        byte[] cover = getAlbumArt(url);
                        if(cover != null){
                            m.setCover(cover);
                        } else {
                            m.setCover(getRandomSongCover(context));
                        }
                        songList.add(m);
                    }
                    cursor.moveToNext();
     ...
     * 随机歌曲封面
     */
    public byte[] getRandomSongCover(Context context) {
        ...
        int resId = res[ThreadLocalRandom.current().nextInt(0, 17 + 1)];
     ...
     * 随机首页专辑封面
     */
    public byte[] getRandomHomeSongListCover(Context context) {
        ...
        int resId = res[ThreadLocalRandom.current().nextInt(0, 4 + 1)];
        Drawable drawable = ContextCompat.getDrawable(context, resId);
        if(drawable != null){
            Bitmap bitmap = ((BitmapDrawable) drawable).getBitmap();
            ByteArrayOutputStream stream = new ByteArrayOutputStream();
     ...
     */
    public @Nullable
    Song next(@NonNull PlayList playList, boolean fromUser) {
        try {
            // 默认使用正序播放列表
     ...
     */
    public @Nullable
    Song previous(@NonNull PlayList playList) {
        try {
            // 默认使用正序播放列表
            List<Song> playSongs = playList.getSongs();
            // 播放方式
